...
//    }
    ...
    @objc private func loginButtonTap(){
        Task{
            do {
                let response = try await LoginService.shared.PostRegisterData(loginId: self.loginId,
                                                                              password: self.password)
                
                pushToMynNickNameVC(userId: response.userId)
...
//                self.present(alert, animated: true)
    ...
    private func pushToMynNickNameVC(userId: Int) {
        let myNickNameViewController = MyNickNameViewController(userId: userId)
        self.navigationController?.pushViewController(myNickNameViewController, animated: true)
    }
